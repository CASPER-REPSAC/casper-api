...
def tester(request):
    ...
    if request.method == "POST":
        return Response('POST method!', status=status.HTTP_201_CREATED)
...
def activity_list(request):
    ...
    if request.method == "GET":
        ...
        author_ = User.objects.filter(id=authed).only("first_name","last_name")
        #for user in author_:
        print(author_)
        #context['request'].POST['author'] = str(user.last_name) + str(user.first_name)
        #print(context['request'].POST['author'])
        #except:
