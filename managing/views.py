...
def activity_list(request):
    ...
    if request.method == "GET":
        ...
        acti = Activity()
        acti.title = request.data['title']

        acti.type = request.data['type']
        acti.author = request.data['author']
        acti.createDate = request.data['createDate']
        acti.description = request.data['description']
        acti.startDate = request.data['startDate']
        acti.endDate = request.data['endDate']
        acti.currentState = request.data['currentState']
        print(dir(acti))
        print(acti.title)
        acti.save()
        #acti.
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            #print(serializer.data.prepend('pk',authed['user_id']))
            #serializer.author = authed['user_id']
            serializer.save(author = authed['user_id'])
            ####start#####
            ### 공사중 ###
        '''
